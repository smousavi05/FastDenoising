...
%-------------------------------------------------------------------------- 
%   S. Mostafa Mousavi 
%   smousavi05@gmail.com
%   Last time modified: Jan, 31, 2019
%---------------------------------------------------------------------------
...
function [dn org opt] = wlSimple(data,opt);
...
%% Thresholding the Coefficients Scale-by-Scale
% Partitioning;
for i = 1:na
    H{i} = wk(i,:);
end
...
for i = 1:na
   ...
   % sigma =  mad(abs(CH(:,1:round(opt.at))))./0.6745; % noise level 
    sigma =  opt.cf*mad(abs(CH(:,1:round(opt.at)))); % noise level 
    switch opt.thr
        case 'hard'
            T_H{i} = HardThresh(H{i},sigma); % hard thresholding 
        case 'soft'
            T_H{i} = SoftThresh(H{i},sigma); % soft thresholding
    end
end
...
% -------------------------------------------------------------------------
t = sig*sqrt(2*log(length(y)));    
x  = y .* (abs(y) > t);
    


function x = SoftThresh(y,sig)
...
function [ dnCuCoef ] = customThrSoft(y,sig)

% Estimate the unuversal thresholding level.

gamma = sqrt(2*log(length(y))) * sig;
% gamma = thselect(wlCoef,'sqtwolog')

% Customized Thresholding
lam = 0.6*gamma; % cutoff value
al = 0.2;         % shape parameter

if abs(y) <= lam
    out = 0;
elseif abs(y) >= gamma
    out = y-sign(y)*(1-al)*gamma;
else
    y1 = al*gamma*((abs(y)-lam)./(gamma-lam)).^2;
    y2 = (al-3)*((abs(y)-lam)./(gamma-lam))+4-al;
    out = y1*y2'; 
end
